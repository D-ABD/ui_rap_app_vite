        ...
        <tbody>
          {documents.map((doc) => (
            <Tr key={`document-${doc.id}`}>
              ...
              <Td>{doc.formation_centre_nom ?? '–'}</Td>
              <Td>{doc.formation_type_offre_libelle ?? '–'}</Td>
              <Td>
      ...
      <CardList>
        {documents.map((doc) => (
          <Card key={`card-${doc.id}`}>
            ...
            <Row>🗂️ {doc.type_document_display} — {doc.taille_readable ?? '–'}</Row>
            <Row>🏢 {doc.formation_centre_nom ?? '–'}</Row>
            <Row>🎯 {doc.formation_type_offre_libelle ?? '–'}</Row>
            <Row>📅 {doc.formation_start_date && doc.formation_end_date
              ? `${doc.formation_start_date} → ${doc.formation_end_date}`
              : '–'}</Row>
            <Row>
              Statut:{' '}
